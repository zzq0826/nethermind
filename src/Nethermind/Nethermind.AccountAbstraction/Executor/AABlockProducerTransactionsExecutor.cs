    ...
    public class AABlockProducerTransactionsExecutor : BlockProcessor.BlockProductionTransactionsExecutor
    {
            ...
            IReleaseSpec spec)
        {
            ...
            foreach (Transaction transaction in transactions)
            {
                ...
                {
                    BlockProcessor.TxAction action = ProcessAccountAbstractionTransaction(block, blkCtx, transaction, i++, executionTracer, processingOptions, transactionsInBlock);
                    if (action == BlockProcessor.TxAction.Stop) break;
                }
                else
                {
                    BlockProcessor.TxAction action = ProcessTransaction(block, blkCtx, transaction, i++, executionTracer, processingOptions, transactionsInBlock);
                    if (action == BlockProcessor.TxAction.Stop) break;
                }
            }
            ...
            LinkedHashSet<Transaction> transactionsInBlock)
        {
            ...
            BlockProcessor.TxAction action = ProcessTransaction(block, blkCtx, currentTx, index, executionTracer, processingOptions, transactionsInBlock, false);
