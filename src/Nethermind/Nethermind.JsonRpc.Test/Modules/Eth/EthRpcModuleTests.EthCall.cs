...
public partial class EthRpcModuleTests
{
    ...
    public async Task Eth_call_web3_sample()
    {
        ...
        string serialized =
            ctx.Test.TestEthRpc("eth_call", ctx.Test.JsonSerializer.Serialize(transaction), "0x0");
    ...
    public async Task Eth_call_web3_should_return_insufficient_balance_error()
    {
        ...
        string serialized = ctx.Test.TestEthRpc("eth_call", ctx.Test.JsonSerializer.Serialize(transaction));
        Assert.AreEqual(
    ...
    public async Task should_not_reject_transactions_with_deployed_code_when_eip3607_enabled()
    {
        ...
        TestSpecProvider specProvider = new(releaseSpec) { NetworkId = ChainId.Mainnet, AllowTestChainOverride = false };
    ...
    {
        using Context ctx = await Context.Create();
        string serialized = ctx.Test.TestEthRpc("eth_call",
