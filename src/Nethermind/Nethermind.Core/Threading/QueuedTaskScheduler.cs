...
// Adapted from
// https://github.com/ChadBurggraf/parallel-extensions-extras/blob/master/TaskSchedulers/QueuedTaskScheduler.cs
// Nuget package either don't work on .net 7 or don't work on linux due to some thread apartment thingi.
    ...
    public sealed class QueuedTaskScheduler : TaskScheduler, IDisposable
    {
        ...
        protected override IEnumerable<Task> GetScheduledTasks()
        {
            ...
            if (_targetScheduler == null)
            {
                ...
                return _blockingTaskQueue!.Where(t => t != null).Select(t => t!).ToList();
            }
            // otherwise get them from the non-blocking queue...
            else
            {
                return _nonthreadsafeTaskQueue!.Where(t => t != null).Select(t => t!).ToList();
            }
        }
