...
[TestFixture]
public class IpaProofStructConverterTests
{
    [Test]
    public void TestRoundTripNew()
    {
        ...
        IpaProofStruct? res = JsonSerializer.Deserialize<IpaProofStruct>(results, options);
        Assert.NotNull(res);

        for (int i = 0; i < item.L.Length; i++)
        {
            Assert.IsTrue(item.L[i] == res!.L[i]);
        }

        for (int i = 0; i < item.R.Length; i++)
        {
            Assert.IsTrue(item.R[i] == res!.R[i]);
        }
    }
}
