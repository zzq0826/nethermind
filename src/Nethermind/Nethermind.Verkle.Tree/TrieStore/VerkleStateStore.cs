...
public partial class VerkleStateStore : IVerkleTrieStore, ISyncTrieStore
{
    ...
    private static Hash256? GetStateRoot(IVerkleDb db)
    {
        return db.GetInternalNode(RootNodeKey, out InternalNode? node) ? new Hash256(node!.Bytes) : null;
    }

    private static Hash256? GetStateRoot(InternalStore db)
    {
        return db.TryGetValue(RootNodeKey, out InternalNode? node) ? new Hash256(node!.Bytes) : null;
    }
    ...
    private void UpdateStateRoot()
    {
        ...
        StateRoot = RootNode == null
            ? Pedersen.Zero
            : new Hash256(RootNode!.InternalCommitment.ToBytes());
    }
}
