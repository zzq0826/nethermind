...
namespace Nethermind.Serialization.Rlp
{
        ...
        where T : Transaction, new()
    {
        ...
        private ulong[] ToULongArray(UInt256 val)
        {
            return new ulong[] { val.u0, val.u1, val.u2, val.u3 };
        }

        private void EncodeEip4844PayloadWithoutPayload(T item, RlpStream stream, RlpBehaviors rlpBehaviors)
        {
            ...
            foreach (System.Collections.Generic.KeyValuePair<Address, System.Collections.Generic.IReadOnlySet<UInt256>> pair in item.AccessList.Data)
            {
        ...
        private void EncodeSignature(T item, RlpStream stream)
        {
        ...
        private void EncodeEip4844Signature(T item, RlpStream stream)
        {
            // TODO: move it to a signature decoder
            if (item.Signature is null)
            {
        ...
        private int GetEip4844ContentLength(T item, bool wrapped)
        {
            int blobVersionedHashesLength = 0;
            for (int i = 0; i < item.BlobVersionedHashes.Length; i++)
            {
        ...
        private int GetSignatureLength(T item, bool forSigning, bool isEip155Enabled = false, int chainId = 0)
        {
            int contentLength = 0;
            if (forSigning)
            {
        ...
        private int GetEip4844SignatureLength(T item, bool forSigning, bool wrapped,  bool isEip155Enabled = false, int chainId = 0)
        {
            int contentLength = 0;
            if (forSigning)
            {
