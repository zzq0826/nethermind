...
internal class StacktraceView : IComponent<DebugTracer>
{
    ...
    {
        public EvmState.ReturnState returnStackEntry { get; set; }

        public override string Text => $"Section: {returnStackEntry.Index} Offset: {returnStackEntry.Offset}";
    }
    ...
    public (View, Rectangle?) View(DebugTracer tracer, Rectangle? rect = null)
    {
        ...
        {
            Index = tracer.CurrentState?.FunctionIndex ?? 0,
            Offset = tracer.CurrentState?.ProgramCounter ?? 0,
            Height = tracer.CurrentState?.DataStackHead ?? 0
        });
