    ...
    private static (string Name, Action<VerkleStateTree, IVerkleStore, IDb> Action)[] InitScenarios()
    {
        return new (string, Action<VerkleStateTree, IVerkleStore, IDb>)[]
        {
            ...
            ("branch_with_same_accounts_at_different_addresses", (tree, stateDb, codeDb) =>
            {
                ...
                tree.InsertStemBatch(account1.Bytes.AsSpan()[..31], AccountJustState0.ToVerkleDict());
                tree.InsertStemBatch(account2.Bytes.AsSpan()[..31], AccountJustState0.ToVerkleDict());
                tree.Commit();
                tree.CommitTree(0);
            }),
            ("just_state", (tree, stateDb, codeDb) =>
            {
                ...
                tree.InsertStemBatch(account1.Bytes.AsSpan()[..31], AccountJustState0.ToVerkleDict());
                tree.InsertStemBatch(account2.Bytes.AsSpan()[..31], AccountJustState1.ToVerkleDict());
                tree.InsertStemBatch(account3.Bytes.AsSpan()[..31], AccountJustState2.ToVerkleDict());
                tree.Commit();
                tree.CommitTree(0);
            })
        };
