...
namespace Nethermind.Synchronization.Test.FastBlocks
{
    ...
    public class FastHeadersSyncTests
    {
        ...
        public async Task Can_prepare_3_requests_in_a_row()
        {
            IDbProvider memDbProvider = await TestMemDbProvider.InitAsync();
            BlockTree blockTree = new(memDbProvider.BlocksDb, memDbProvider.HeadersDb, memDbProvider.BlockInfosDb, new ChainLevelInfoRepository(memDbProvider.BlockInfosDb), MainnetSpecProvider.Instance, NullBloomStorage.Instance, LimboLogs.Instance);
            HeadersSyncFeed feed = new(Substitute.For<ISyncModeSelector>(), blockTree, Substitute.For<ISyncPeerPool>(), new SyncConfig { FastSync = true, PivotNumber = "1000", PivotHash = Keccak.Zero.ToString(), PivotTotalDifficulty = "1000" }, Substitute.For<ISyncReport>(), LimboLogs.Instance);
        ...
        public async Task Can_prepare_several_request_and_ignore_request_from_previous_sequence()
        {
            ...
            ResettableHeaderSyncFeed feed = new(Substitute.For<ISyncModeSelector>(), blockTree, Substitute.For<ISyncPeerPool>(), new SyncConfig { FastSync = true, PivotNumber = "500", PivotHash = pivot.Hash.Bytes.ToHexString(), PivotTotalDifficulty = pivot.TotalDifficulty!.ToString() }, syncReport, LimboLogs.Instance);
            feed.InitializeFeed();
        ...
        public async Task Can_reset_and_not_hang_when_a_batch_is_processing()
        {
            ...
            ResettableHeaderSyncFeed feed = new(
                ...
                blockTree,
                Substitute.For<ISyncPeerPool>(),
                new SyncConfig { FastSync = true, PivotNumber = "500", PivotHash = pivot.Hash.Bytes.ToHexString(), PivotTotalDifficulty = pivot.TotalDifficulty!.ToString() },
                syncReport,
                LimboLogs.Instance,
        ...
        public async Task Finishes_when_all_downloaded()
        {
            ...
            HeadersSyncFeed feed = new(Substitute.For<ISyncModeSelector>(), blockTree, Substitute.For<ISyncPeerPool>(), new SyncConfig { FastSync = true, PivotNumber = "1000", PivotHash = Keccak.Zero.ToString(), PivotTotalDifficulty = "1000" }, report, LimboLogs.Instance);
            await feed.PrepareRequest();
