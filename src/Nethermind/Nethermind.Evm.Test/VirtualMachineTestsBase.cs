...
namespace Nethermind.Evm.Test
{
    public class VirtualMachineTestsBase
    {
        ...
        public virtual void Setup()
        {
            ...
            _processor = new TransactionProcessor(SpecProvider, TestState, Storage, Machine,
                NullBlockFinder.Instance, logManager);
        }
        ...
        protected void AssertStorage(StorageCell storageCell, UInt256 expectedValue)
        {
            ...
            {
                Assert.AreEqual(expectedValue.ToBigEndian().WithoutLeadingZeros().ToArray(), new byte[] { 0 },
                    $"storage {storageCell}, call {_callIndex}");
            }
            else
            {
                byte[] actualValue = Storage.Get(storageCell);
                Assert.AreEqual(expectedValue.ToBigEndian().WithoutLeadingZeros().ToArray(), actualValue,
                    $"storage {storageCell}, call {_callIndex}");
            }
        }
