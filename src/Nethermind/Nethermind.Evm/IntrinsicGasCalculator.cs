...
public static class IntrinsicGasCalculator
{
    ...
    public static UInt256 CalculateDataGasPrice(Transaction transaction, UInt256 parentExcessDataGas,
        ...
        CalculateDataGas(transaction.BlobVersionedHashes?.Length ?? 0, releaseSpec) * GetDataGasPrice(parentExcessDataGas);

    public static UInt256 CalculateDataGas(int blobCount, IReleaseSpec releaseSpec) =>
        !releaseSpec.IsEip4844Enabled ? (UInt256)0 : ((ulong)blobCount * Eip4844Constants.DataGasPerBlob);
