    ...
    public static class CodeDepositHandler
    {
        ...
        public static bool CreateCodeIsValid(ICodeInfo codeInfo, ReadOnlyMemory<byte> initCode, IReleaseSpec spec)
        {
            if (spec.IsEip3540Enabled)
            {
                ...
                ReadOnlySpan<byte> initCodeSpan = initCode.Span;
                if (version > 0 && version != EvmObjectFormat.GetCodeVersion(initCodeSpan))
                    return false;
                if (EvmObjectFormat.IsEof(initCodeSpan)) // this needs test cases
