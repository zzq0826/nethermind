...
namespace Nethermind.Evm.Precompiles
{
    public class PointEvaluationPrecompile : IPrecompile
    {
        ...
        public (ReadOnlyMemory<byte>, bool) Run(in ReadOnlyMemory<byte> inputData, IReleaseSpec releaseSpec)
        {
            ...
            if (!KzgPolynomialCommitments.VerifyHashV1Format(versionedHash))
            {
                return (ReadOnlyMemory<byte>.Empty, false);
            }

            ReadOnlySpan<byte> x = inputData.Span[32..64];

            if (new UInt256(x) >= KzgPolynomialCommitments.BlsModulus)
            {
                return (ReadOnlyMemory<byte>.Empty, false);
            }

            ReadOnlySpan<byte> y = inputData.Span[64..96];
            if (new UInt256(y) >= KzgPolynomialCommitments.BlsModulus)
            {
                return (ReadOnlyMemory<byte>.Empty, false);
            }

            var commitment = inputData.Span[96..144];
            ...
            if (!KzgPolynomialCommitments.VerifyProof(commitment, x, y, proof))
            {
                return (ReadOnlyMemory<byte>.Empty, false);
            }
