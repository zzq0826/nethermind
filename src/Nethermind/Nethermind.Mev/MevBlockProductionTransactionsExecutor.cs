...
namespace Nethermind.Mev
{
    ...
    public class MevBlockProductionTransactionsExecutor : BlockProcessor.BlockProductionTransactionsExecutor
    {
        ...
        public override TxReceipt[] ProcessTransactions(Block block, ProcessingOptions processingOptions, BlockExecutionTracer executionTracer, IReleaseSpec spec)
        {
            ...
            foreach (Transaction currentTx in transactions)
            {
                ...
                if (bundleHash is null)
                {
                    ...
                    else
                    {
                        ...
                        TxAction action = ProcessTransaction(block, blkCtx, currentTx, transactionsInBlock.Count, executionTracer, processingOptions, transactionsInBlock);
                ...
                else
                {
                    ...
                    if (currentTx is BundleTransaction bundleTransaction)
                    {
                        ...
                        else
                        {
                            ...
                            TxAction action = ProcessBundle(block, blkCtx, bundleTransactions, transactionsInBlock, executionTracer, processingOptions);
                            if (action == TxAction.Stop) break;
                    ...
                    else
                    {
                        ...
                        action = ProcessTransaction(block, blkCtx, currentTx, transactionsInBlock.Count, executionTracer, processingOptions, transactionsInBlock);
                        if (action == TxAction.Stop) break;
                    }
                }
            }
            ...
            {
                ProcessBundle(block, blkCtx, bundleTransactions, transactionsInBlock, executionTracer, processingOptions);
            }
            ...
            ProcessingOptions processingOptions)
        {
            ...
            {
                txAction = ProcessBundleTransaction(block, blkCtx, bundleTransactions[index], index, executionTracer, processingOptions, transactionsInBlock);
        ...
        private TxAction ProcessBundleTransaction(
            ...
            LinkedHashSet<Transaction> transactionsInBlock)
        {
            TxAction action = ProcessTransaction(block, blkCtx, currentTx, index, executionTracer, processingOptions, transactionsInBlock, false);
