...
namespace Nethermind.Merge.Plugin.Synchronization
{
    public class MergeBlockDownloader : BlockDownloader
    {
            ...
            CancellationToken cancellation)
        {
            ...
            {
                if (_logger.IsInfo)
                    _logger.Info($"Continue full sync with {bestPeer} (our best {_blockTree.BestKnownNumber})");
                ...
                {
                    if (_logger.IsInfo)
                        _logger.Info("Chain level helper got no headers suggestion");
                    return false;
                }
            ...
            while (HasMoreToSync(out BlockHeader[]? headers, out int headersToRequest))
            {
                ...
                {
                    if (_logger.IsInfo)
                        _logger.Info("Break early due to no blocks.");
                    break;
                }
