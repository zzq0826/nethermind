...
namespace Nethermind.Store.Test
{
    ...
    public class StateReaderTests
    {
        ...
        public void Non_existing()
        {
            ...
            StateReader reader =
                new(new TrieStore(stateDb, LimboLogs.Instance), Substitute.For<IDb>(), Logger);
            reader.GetStorage(stateRoot0, new StorageCell(_address1, storageCell.Index + 1)).Should().BeEquivalentTo(new byte[] { 0 });
            // reader.GetStorage(Keccak.EmptyTreeHash, new StorageCell(_address1, storageCell.Index + 1)).Should().BeEquivalentTo(new byte[] { 0 });
        }
        ...
        private Task StartStorageTask(StateReader reader, Hash256 stateRoot, StorageCell storageCell, byte[] value)
        {
            return Task.Run(
                () =>
                {
                    ...
                    {
                        byte[] result = reader.GetStorage(stateRoot, storageCell);
        ...
        public async Task Get_storage()
        {
            ...
            var retrieved = reader.GetStorage(state.StateRoot, new StorageCell(_address1, storageCell.Index));
            ...
            retrieved =
                reader.GetStorage(processorStateProvider.StateRoot, storageCell);
