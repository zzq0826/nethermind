...
{
    public interface ITrieBlendStore : ITrieStore
    {
        IReadOnlyTrieBlendStore AsReadOnly(IKeyValueStore? keyValueStore, IKeyValueStore? leafKeyValueStore);
    }

    public interface IReadOnlyTrieBlendStore : IReadOnlyTrieStore
    {
        IReadOnlyTrieBlendStore AsReadOnly(IKeyValueStore? keyValueStore, IKeyValueStore? leafKeyValueStore);
    }

    public class TrieBlendStore : ITrieBlendStore
    {
        private class DirtyNodesCache
        {
            private readonly TrieBlendStore _trieStore;

            public DirtyNodesCache(TrieBlendStore trieStore)
            {
        ...
        private IBatch? _currentAccountBatch = null;

        private readonly ConcurrentDictionary<long, BlockDataLayer> _blockLayers;

        private readonly DirtyNodesCache _dirtyNodes;

        public TrieBlendStore(
            IKeyValueStoreWithBatching? keyValueStore,
            IKeyValueStoreWithBatching? accountValueStore,
            ...
            ILogManager? logManager)
        {
            ...
            _leafFlatStore = accountValueStore ?? throw new ArgumentNullException(nameof(accountValueStore));
            _pruningStrategy = pruningStrategy ?? throw new ArgumentNullException(nameof(pruningStrategy));
            _persistenceStrategy = persistenceStrategy ?? throw new ArgumentNullException(nameof(persistenceStrategy));
            _blockLayers = new ConcurrentDictionary<long, BlockDataLayer>();
        ...
        public void CommitNode(long blockNumber, NodeCommitInfo nodeCommitInfo)
        {
            ...
            if (!nodeCommitInfo.IsEmptyBlockMarker && !nodeCommitInfo.Node.IsBoundaryProofNode)
            {
                ...
                Persist(node, nodeCommitInfo.FullPath, blockNumber);
        ...
        internal byte[] LoadRlp(Keccak keccak, IKeyValueStore? keyValueStore)
        {
            ...
            if (rlp == null)
            {
                return _leafFlatStore[keccak.Bytes];
            }

            RlpStream rlpStream = new(rlp);
            rlpStream.ReadSequenceLength();
            int numberOfItems = rlpStream.ReadNumberOfItemsRemaining(null, 4);
            if (numberOfItems == 3)
            {
                return ConvertLeafPointer(rlpStream);
            }
        ...
        {
            return new ReadOnlyTrieBlendStore(this, keyValueStore, _leafFlatStore);
        }

        public IReadOnlyTrieBlendStore AsReadOnly(IKeyValueStore? keyValueStore, IKeyValueStore? leafKeyValueStore)
        {
            return new ReadOnlyTrieBlendStore(this, keyValueStore, leafKeyValueStore);
        }
        ...
        private readonly IKeyValueStoreWithBatching _leafFlatStore;
        ...
        public static byte[] EncodeLeafNodePointer(HexPrefix nodeKey, byte[] fullPath, long blockNumber)
        {
            byte[] keyBytes = nodeKey.ToBytes();
            int contentLength = Rlp.LengthOf(keyBytes) + Rlp.LengthOf(blockNumber) + Rlp.LengthOf(fullPath);
            int totalLength = Rlp.LengthOfSequence(contentLength);
            RlpStream rlpStream = new(totalLength);
            rlpStream.StartSequence(contentLength);
            rlpStream.Encode(keyBytes);
            rlpStream.Encode(blockNumber);
            rlpStream.Encode(fullPath);
        ...
        private void PersistOnShutdown()
        {
            ...
            foreach (var layer in _blockLayers)
            {
                if (!layer.Value.IsPersisted)
                    layer.Value.Persist(null);
            }
        }
        ...
        private byte[] EncodeAccountLeafNode(HexPrefix key, byte[] nodeValueRLP)
        {
            byte[] keyBytes = key.ToBytes();
            int contentLength = Rlp.LengthOf(keyBytes) + Rlp.LengthOf(nodeValueRLP);
            int totalLength = Rlp.LengthOfSequence(contentLength);
            RlpStream repacked = new(totalLength);
            repacked.StartSequence(contentLength);
            repacked.Encode(keyBytes);
            repacked.Encode(nodeValueRLP);
            return repacked.Data;
        }

        private BlockDataLayer GetLayer(long blockNumber)
        {
            if (_blockLayers.TryGetValue(blockNumber, out BlockDataLayer layer))
            {
                return layer;
            }
            else
            {
                layer = new BlockDataLayer(_leafFlatStore, blockNumber);
                layer.LoadLog(blockNumber);
                _blockLayers.TryAdd(blockNumber, layer);
                return layer;
            }
        }

        private byte[] ConvertLeafPointer(RlpStream rlpStream)
        {
            HexPrefix key = HexPrefix.FromBytes(rlpStream.DecodeByteArraySpan());
            if (key.IsLeaf)
            {
                long blockNo = rlpStream.DecodeLong();
                byte[] fullPath = rlpStream.DecodeByteArray();
                BlockDataLayer layer = GetLayer(blockNo);
                byte[] accountRLP = layer[fullPath];

                return EncodeAccountLeafNode(key, accountRLP);
            }
            else
            {
                //should never happen
                throw new InvalidOperationException("Non-leaf node stored in flat storage!");
            }
        }
        ...
        public void SaveNode(long blockNumber, byte[] fullPath, TrieNode trieNode, Keccak nodeHash = null)
        {
            ...
            if (trieNode.IsLeaf)
            {
                if (_logger.IsTrace)
                {
                    _logger.Trace($"Saving node {fullPath.ToHexString()} at block {blockNumber}, kecckak: {hash}");
                }

                byte[] pointer = EncodeLeafNodePointer(trieNode.Key, fullPath, blockNumber);
                _leafFlatStore[fullPath] = trieNode.Value;
                _keyValueStore[hash.Bytes] = pointer;
            }
            else
            {
                _keyValueStore[hash.Bytes] = trieNode.FullRlp;
            }
        }
    }
}
