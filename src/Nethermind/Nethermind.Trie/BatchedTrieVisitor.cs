...
public class BatchedTrieVisitor
{
    ...
    private ArrayPool<(TreePath, TrieNode, SmallTrieVisitContext)> _trieNodePool = ArrayPool<(TreePath, TrieNode, SmallTrieVisitContext)>.Create();
        ...
        if (_activeJobs < _targetCurrentItems)
        {
            lock (theStack)
            {
                for (int i = 0; i < _maxBatchSize; i++)
                {
                    ...
                    if (item.Context.Storage != null)
                    {
                        resolver = resolver.GetStorageTrieNodeResolver(item.Context.Storage);
                    }

                    finalBatch.Add((item.Key.Item1, resolver.FindCachedOrUnknown(item.Key.Item1, item.Key.Item2.ToCommitment()), item.Context));
    ...
    {
        public readonly (TreePath, ValueHash256) Key;
