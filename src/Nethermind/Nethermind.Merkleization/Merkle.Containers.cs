...
namespace Nethermind.Merkleization
{
    ...
    public static partial class Merkle
    {
        ...
        public static void Ize(out UInt256 root, Transaction transaction)
        {
            ...
            merkleizer.Feed(transaction.Nonce.ToDirichlet());
            merkleizer.Feed(transaction.GasPrice.ToDirichlet());
            merkleizer.Feed(transaction.DecodedMaxFeePerGas.ToDirichlet());
            merkleizer.Feed((ulong)transaction.GasLimit);
            merkleizer.FeedOptional(transaction.To);
            merkleizer.Feed(transaction.Value.ToDirichlet());
            Ize(out UInt256 dataRoot, transaction.Data ?? new byte[0], 1 << 24);
            MixIn(ref dataRoot, transaction.Data?.Length ?? 0);
            merkleizer.Feed(dataRoot);
            merkleizer.Feed(transaction.AccessList);
            merkleizer.Feed((transaction.MaxFeePerDataGas ?? 0).ToDirichlet());
