...
namespace Nethermind.Crypto
{
    ...
    public class Ecdsa : IEcdsa
    {
        public Signature Sign(PrivateKey privateKey, Keccak message)
        {
            ...
            byte[] signatureBytes = Proxy.SignCompact(message.GetThreadStaticByteBuffer, privateKey.KeyBytes, out int recoveryId);
        ...
        public PublicKey? RecoverPublicKey(Signature signature, Keccak message)
        {
            ...
            bool success = Proxy.RecoverKeyFromCompact(publicKey, message.GetThreadStaticByteBuffer, signature.Bytes, signature.RecoveryId, false);
            if (!success)
            {
                return null;
            }
        ...
        public CompressedPublicKey? RecoverCompressedPublicKey(Signature signature, Keccak message)
        {
            ...
            bool success = Proxy.RecoverKeyFromCompact(publicKey, message.GetThreadStaticByteBuffer, signature.Bytes, signature.RecoveryId, true);
            if (!success)
            {
                return null;
            }
