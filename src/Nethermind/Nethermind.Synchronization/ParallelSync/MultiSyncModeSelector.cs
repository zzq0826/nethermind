...
namespace Nethermind.Synchronization.ParallelSync
{
    ...
    public class MultiSyncModeSelector : ISyncModeSelector, IDisposable
    {
        ...
        private bool ShouldBeInStateNodesMode(Snapshot best)
        {
            ...
            bool rangesFinished = _syncProgressResolver.IsGetRangesFinished();

            bool result = isInStateSync && (snapSyncDisabled || rangesFinished) && (verkleSyncDisabled || rangesFinished);
            ...
            if (_logger.IsTrace)
            {
                LogDetailedSyncModeChecks("STATE_NODES",
                    ...
                    (nameof(rangesFinished), rangesFinished));
            }
        ...
        private ref struct Snapshot
        {
            ...
            )
            {
                ...
                IsInWaitingForBlock = IsInDisconnected = IsInFastReceipts = IsInFastBodies = IsInFastHeaders
                    = IsInFastSync = IsInFullSync = IsInStateSync = IsInStateNodes = IsInSnapRanges = IsInVerkleRanges
                        = IsInBeaconHeaders = false;
            }
