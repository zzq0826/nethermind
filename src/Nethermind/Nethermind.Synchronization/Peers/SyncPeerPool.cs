    ...
    public class SyncPeerPool : ISyncPeerPool, IPeerDifficultyRefreshPool
    {
        ...
        public void RemovePeer(ISyncPeer syncPeer)
        {
            ...
            foreach ((SyncPeerAllocation allocation, _) in _replaceableAllocations)
            {
                ...
                {
                    if (_logger.IsInfo) _logger.Info($"Requesting peer cancel with {syncPeer.Node:c} on {allocation}");
                    allocation.Cancel();
                }
            }
...
#pragma warning restore 4014
                {
                    ...
                    if (t.IsFaulted)
                    {
                        ...
                        {
                            if (_logger.IsInfo) _logger.Info($"Refreshing info for {syncPeer} failed due to timeout: {t.Exception.Message}");
                        }
                    ...
                    {
                        if (_logger.IsInfo) _logger.Info($"Refresh peer info canceled: {syncPeer.Node:s}");
                    }
                    else
                    {
                        UpgradeAllocations();
        ...
        internal void DropUselessPeers(bool force = false)
        {
            ...
            if (_logger.IsInfo) _logger.Info($"Reviewing {PeerCount} peer usefulness");
